...
namespace crypto {
    int generate_keypair(std::string input, std::vector<std::string>& keypair, unsigned char* seckey,
        ...
        std::string prefix = "generate_keypair: ";
        ...
        // Generate secret key.
        Logger::instance().debug("{}Generate secret key.", prefix);
        if (!fill_random(seckey, STD_SIZE)) {
            Logger::instance().error("{}Failed to generate randomness.", prefix);
            return EXIT_FAILURE;
        }
        else {
            Logger::instance().debug("{}Secret key generated successfully.\n", prefix);
        }

        // Verify secret key.
        Logger::instance().debug("{}Verify secret key.", prefix);
        if (!secp256k1_ec_seckey_verify(ctx, seckey)) {
            Logger::instance().error("{}Generated secret key is invalid. This indicates an issue with the random number generator.\n", prefix);
            return EXIT_FAILURE;
        }
        else {
            Logger::instance().debug("{}Secret key verified successfully.\n", prefix);
        }

        /* public key creation using a valid context with a verified secret key should never fail */
        Logger::instance().debug("{}Create public key.", prefix);
        return_val = secp256k1_ec_pubkey_create(ctx, &pubkey, seckey);
        if (return_val == 0)
            return EXIT_FAILURE;
        else
            Logger::instance().debug("{}Public key created successfully.\n", prefix);

        /* serialize the pubkey in a compressed form(33 bytes). should always return 1. */
        len = STD_SIZE + 1;
        return_val = secp256k1_ec_pubkey_serialize(ctx, compressed_pubkey, &len, &pubkey, SECP256K1_EC_COMPRESSED);
        if (return_val == 0) {
            Logger::instance().error("{}Failed to serialize public key.", prefix);
            return EXIT_FAILURE;
        }

        /* should be the same size as the size of the output, because we passed a 33 byte array. */
        if (len != STD_SIZE + 1) {
            Logger::instance().error("{}Len of public key not equal 33 bytes. Len: {}", prefix, len);
            return EXIT_FAILURE;
        }
    ...
    int create_address_from_pubkey(unsigned char* compressed_pubkey, unsigned char* ripemd160_hash,
        ...
        std::string prefix = "create_address_from_pubkey: ";
        const int STD_SIZE = 32;
        const int RIPEMD160_SIZE = 20;

        std::vector<unsigned char> compressed_pubkey_hash_vector(STD_SIZE);

        Logger::instance().debug("{}Generate address for public key.", prefix);
        ...
        if ((bytes_to_hex(ripemd160_hash, RIPEMD160_SIZE)).length() / 2 != RIPEMD160_SIZE) {
            Logger::instance().error("{}Failed to generate address for public key.", prefix);
            return EXIT_FAILURE;
        }
    ...
    int generate_sign(std::string input, unsigned char* seckey, unsigned char* serialized_signature,
        ...
        std::string prefix = "generate_sign: ";
        const int STD_SIZE = 32;
        ...
        Logger::instance().debug("{}Generate hash for message.", prefix);
        ...
        if (return_val == 0) {
            Logger::instance().error("{}Failed to generate signature.", prefix);
            return EXIT_FAILURE;
        }
        ...
        * according to the documentation in secp256k1.h. */
        ...
        if (return_val == 0) {
            Logger::instance().error("{}Failed to serialize signature.", prefix);
            return EXIT_FAILURE;
        }
    ...
    int verify_sign(secp256k1_ecdsa_signature& sig, unsigned char* serialized_signature,
        ...
        if (!secp256k1_ec_pubkey_parse(ctx, &pubkey, compressed_pubkey, STD_SIZE + 1)) {
            Logger::instance().error("{}Failed parsing the public key.n", prefix);
            return EXIT_FAILURE;
        }
