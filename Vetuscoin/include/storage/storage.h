...
namespace storage {
    ...
    */
    ...
    /* @brief           Encode a chain as bytes: block count (varint) followed by each block's
    *                   Serialize() back to back. Shared by save_chain() (writes to a file) and
    *                   the network layer's CHAIN message (writes to a socket) - one format,
    *                   two carriers.
    *  @param   chain   blocks to encode, in order.
    *  @return          encoded bytes.
    */
    inline std::vector<uint8_t> serialize_chain(const std::vector<block::CBlock>& chain) {
    ...
    /* @brief           Decode a chain from bytes written by serialize_chain().
    *  @param   buffer  encoded bytes.
    *  @return          blocks in the order they were encoded.
    */
    inline std::vector<block::CBlock> deserialize_chain(const std::vector<uint8_t>& buffer) {
        size_t offset = 0;
    ...
    /* @brief           Save the whole chain to a binary file (see serialize_chain()).
    ...
    */
    inline void save_chain(const std::vector<block::CBlock>& chain, const std::string& path) {
        ...
        std::vector<uint8_t> buffer = serialize_chain(chain);
    ...
    */
    inline std::vector<block::CBlock> load_chain(const std::string& path) {
        ...
        std::vector<block::CBlock> chain = deserialize_chain(read_file(path));
    ...
    *  @param   path    destination file path.
    */
    inline void save_utxo_set(const transaction::UtxoSet& utxos, const std::string& path) {
    ...
    */
    inline transaction::UtxoSet load_utxo_set(const std::string& path) {
